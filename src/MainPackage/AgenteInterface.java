...
public class AgenteInterface {
		...
		public void startAUInPlatform(String name, String classpath,HashMap<Double,AEInfo> incentivos,ArrayList<AEInfo> ael) {
			Random random = new Random();
			int coords[]=new int[2];
			coords[0]=random.nextInt(100 - 0 + 1) + 0;
			coords[1]=random.nextInt(100 - 0 + 1) + 0;			
			
			AUInfo au=new AUInfo(coords);
			try {
		...
		public static void main(String[] args) throws InterruptedException {
			...
			AgenteInterface a = new AgenteInterface();
			a.initMainContainerInPlatform("localhost", "9888", "Interface");
			...
			while (AUnum>0) {
				...
				a.startAUInPlatform("AU"+AUnum, "MainPackage.AgenteUtilizador",incentivos,ael);
				AUnum--;
			}
			while(true){
				TimeUnit.SECONDS.sleep(2);
				for(AEInfo ae : ael){
					System.out.println(ae.getAENum()+" nivel: "+ae.getNivel());
				}
				System.out.println();			
			}
			/*
